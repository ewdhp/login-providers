...
router.all('/:platform/:action', tokenMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (typeof service[action] !== 'function') {
      return ResponseHandler.notFound(
        res,
        `The action "${action}" is not available for the platform "
        ${platform}". Please check the documentation for available actions.`
      );
    }
    ...
    return ResponseHandler.success(
      res,
      `${platform} ${action} executed successfully`,
      { data: result }
    );
  } catch (error) {
    console.error(`Error executing ${action} for ${platform}:`, error.stack);
